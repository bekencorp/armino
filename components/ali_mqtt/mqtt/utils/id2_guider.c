...
static void id2_guider_get_timestamp_str(char *buf, int len)
{
    ...
    if (retry > 1) {
        log_err("utils_get_epoch_time_from_ntp() retry = %d.\n", retry);
    }
    ...
    uint16_t *pport)
{
    ...
    */
    ...
    if (NULL == pvalue) {
        cipher_data = 1;
        log_debug("'data.iotId' NOT found, cipher_data = %d\n", cipher_data);
    } else {
        LITE_free(pvalue);
        pvalue = NULL;
        cipher_data = 0;
        log_debug("'data.iotId' already found, cipher_data = %d\n", cipher_data);
    }
    ...
    if (cipher_data) {
        ...
        id2_rc = utils_base64decode((const uint8_t *)pvalue,
                                    ...
                                    b64_decode,
                                    &dst_len);
        log_debug("rc = utils_base64decode() = %d, %u Bytes => %u Bytes.\n", id2_rc, src_len, dst_len);
        if (id2_rc) {
            goto do_exit;
        }
        LITE_free(pvalue);
        ...
        id2_rc = tfs_id2_decrypt((const uint8_t *)b64_decode,
                                 ...
                                 &dec_len);
        /* tobe remove */
        log_debug("rc = tfs_id2_decrypt() = %d, %u Bytes => %u Bytes.\n", id2_rc, dst_len, dec_len);
        if (id2_rc != 0) {
