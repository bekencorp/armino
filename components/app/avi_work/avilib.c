...
static int avi_add_chunk(avi_t *AVI, char *tag, void *data, int length)
{
	unsigned char c[8];
	int length_org = length, length_add = 0;

	length = PAD_EVEN(length);
	length_add = length - length_org;
	...
	/* add extral data is length not 4byte align*/
	if (length_add) {
		memset(c, 0, length_add);
		if (write(AVI->fdes, c, length_add) != length_add) {
			lseek(AVI->fdes, AVI->pos, SEEK_SET);
			AVI_errno = AVI_ERR_WRITE;
			return -1;
		}
...
static int avi_add_index_entry(avi_t *AVI, char *tag, long flags, long pos, long len)
{
	...
	if (AVI->n_idx >= AVI->max_idx) {
		//     ptr = AVILIB_REALLOC((void *)AVI->idx,(AVI->max_idx+4096)*16);
		ptr = rt_realloc((void *)AVI->idx, (AVI->max_idx + 4096) * 16);
		if (ptr == 0) {
	...
	/* Add index entry */
	//offset is begin after "movi"::AVI->pos-HEADERBYTES+4
	pos = pos - HEADERBYTES + 4;
...
void AVI_open_output_file(avi_t **avi_p, char *filename)
{
	...
	if (NULL == *avi_p) {
		AVI = (avi_t *) AVILIB_MALLOC(sizeof(avi_t));
		if (NULL == AVI) {
			AVI_errno = AVI_ERR_NO_MEM;
			return;
		}
	...
	AVI_header = (unsigned char *)AVILIB_MALLOC(sizeof(unsigned char) * HEADERBYTES);
	if (AVI_header == NULL) {
		AVI_errno = AVI_ERR_NO_MEM;
		AVILIB_FREE(AVI);
		*avi_p = NULL;
	...
	AVI->fdes = open(filename, O_RDWR | O_CREAT | O_BINARY | O_TRUNC, 0600);
	if (AVI->fdes < 0) {
		AVI_errno = AVI_ERR_OPEN;
		AVILIB_FREE(AVI);
		AVILIB_FREE(AVI_header);
		*avi_p = NULL;
		return;
	}
...
#define AVIF_COPYRIGHTED        0x00020000
	...
	OUTLONG(1280);         /* Width *///AVI->width
	OUTLONG(720);        /* Height *///AVI->height
...
static int avi_write_data(avi_t *AVI, char *data, long length, int audio)
{
	...
	if (audio)
		...
		n = avi_add_index_entry(AVI, "00dc", 0x10, AVI->pos, length);
	...
	if (audio)
		...
		n = avi_add_chunk(AVI, "00dc", data, length);
...
int AVI_dup_frame(avi_t *AVI)
{
	...
	if (avi_add_index_entry(AVI, "00dc", 0x10, AVI->last_pos, AVI->last_len))
		return -1;
	AVI->video_frames++;
...
int AVI_close(avi_t *AVI)
{
	...
	if (AVI->idx)
		AVILIB_FREE(AVI->idx);
	if (AVI->video_index)
		AVILIB_FREE(AVI->video_index);
	if (AVI->audio_index)
		AVILIB_FREE(AVI->audio_index);
	AVILIB_FREE(AVI);
...
avi_t *AVI_open_input_file(const char *filename, int getIndex)
{
	...
	if (read(AVI->fdes, data, 12) != 12)
		...
		if (strncasecmp((const char *)data, (const char *)"RIFF", 4) != 0 ||
			strncasecmp((const char *)(data + 8), (const char *)"AVI ", 4) != 0)
	...
	while (1) {
		...
		if (strncasecmp((const char *)data, (const char *)"LIST", 4) == 0) {
			if (read(AVI->fdes, data, 4) != 4)
				ERR_EXIT(AVI_ERR_READ)

				n -= 4;

			if (strncasecmp((const char *)data, (const char *)"hdrl", 4) == 0) {
				hdrl_len = n;
				hdrl_data = (unsigned char *) AVILIB_MALLOC(n);
				if (hdrl_data == 0)
					ERR_EXIT(AVI_ERR_NO_MEM)
					if (read(AVI->fdes, hdrl_data, n) != n)
						ERR_EXIT(AVI_ERR_READ)
					} else if (strncasecmp((const char *)data, (const char *)"movi", 4) == 0) {
				AVI->movi_start = lseek(AVI->fdes, 0, SEEK_CUR);
				lseek(AVI->fdes, n, SEEK_CUR);
			} else
				lseek(AVI->fdes, n, SEEK_CUR);
		} else if (strncasecmp((const char *)data, (const char *)"idx1", 4) == 0) {
			/* n must be a multiple of 16, but the reading does not
			break if this is not the case */

			AVI->n_idx = AVI->max_idx = n / 16;
			AVI->idx = (unsigned  char((*)[16])) AVILIB_MALLOC(n);
			if (AVI->idx == 0)
				ERR_EXIT(AVI_ERR_NO_MEM)

				if (read(AVI->fdes, AVI->idx, n) < 0)
					ERR_EXIT(AVI_ERR_READ)
				} else
			lseek(AVI->fdes, n, SEEK_CUR);
	}
	...
	if (!hdrl_data)
		...
		if (!AVI->movi_start)
			...
			for (i = 0; i < hdrl_len;) {
				...
				if (strncasecmp((const char *)(hdrl_data + i), (const char *)"LIST", 4) == 0) {
					i += 12;
					continue;
				}
				...
				if (strncasecmp((const char *)(hdrl_data + i), (const char *)"strh", 4) == 0) {
					i += 8;
					if (strncasecmp((const char *)(hdrl_data + i), (const char *)"vids", 4) == 0 && !vids_strh_seen) {
						...
						vids_strh_seen = 1;
						lasttag = 1; /* vids */
					} else if (strncasecmp((const char *)(hdrl_data + i), (const char *)"auds", 4) == 0 && ! auds_strh_seen) {
						...
						lasttag = 0;
					num_stream++;
				} else if (strncasecmp((const char *)(hdrl_data + i), (const char *)"strf", 4) == 0) {
					i += 8;
					if (lasttag == 1) {
	...
	if (AVI->idx) {
		...
		for (i = 0; i < AVI->n_idx; i++)
			if (strncasecmp((const char *)AVI->idx[i], (const char *)AVI->video_tag, 3) == 0)
				break;
		...
		if (read(AVI->fdes, data, 8) != 8)
			ERR_EXIT(AVI_ERR_READ)
			if (strncasecmp((const char *)data, (const char *)AVI->idx[i], 4) == 0 && str2ulong(data + 4) == len) {
	...
	if (idx_type == 0) {
		...
		while (1) {
			...
			if (((data[2] == 'd' || data[2] == 'D') &&
				...
				|| ((data[2] == 'w' || data[2] == 'W') &&
					(data[3] == 'b' || data[3] == 'B')))
				avi_add_index_entry(AVI, (char *)data, 0, lseek(AVI->fdes, 0, SEEK_CUR) - 8, n);
	...
	for (i = 0; i < AVI->n_idx; i++) {
		if (strncasecmp((const char *)AVI->idx[i], (const char *)AVI->video_tag, 3) == 0)
			nvi++;
		if (strncasecmp((const char *)AVI->idx[i], (const char *)AVI->audio_tag, 4) == 0)
			nai++;
	}
	...
	if (AVI->video_frames == 0)
		ERR_EXIT(AVI_ERR_NO_VIDS)
		AVI->video_index = (video_index_entry *) AVILIB_MALLOC(nvi * sizeof(video_index_entry));
	if (AVI->video_index == 0)
		ERR_EXIT(AVI_ERR_NO_MEM)
		if (AVI->audio_chunks) {
			AVI->audio_index = (audio_index_entry *) AVILIB_MALLOC(nai * sizeof(audio_index_entry));
...
long AVI_read_frame(avi_t *AVI, char *vidbuf, long bytes)
{
	...
	while (bytes > 0) {
		left = AVI->video_index[AVI->video_pos].len - AVI->video_posb;
		if (left == 0) {
			AVI->video_pos++;
			AVI->video_posb = 0;
		...
		pos = AVI->video_index[AVI->video_pos].pos + AVI->video_posb;
		lseek(AVI->fdes, pos, SEEK_SET);
		if (read(AVI->fdes, vidbuf + nr, todo) < 0) {
...
long AVI_read_audio(avi_t *AVI, char *audbuf, long bytes)
{
	...
	while (bytes > 0) {
		...
		lseek(AVI->fdes, pos, SEEK_SET);
		if (read(AVI->fdes, audbuf + nr, todo) < 0) {
			AVI_errno = AVI_ERR_READ;
			return -1;
		}
		bytes -= todo;
