...
uint8_t *h264_dest = (uint8_t *)0x62000000;
...
static void yuv_buf_set_config(void)
{
	yuv_buf_config.work_mode = YUV_BUF_MODE_H264;
	yuv_buf_config.mclk_div = YUV_MCLK_DIV_3;
...
void h264_line_down_handler(void)
{
	...
	delay(10);
	REG_WRITE(0x48020000+0x0f*4,0x1);
#endif
}
...
void h264_dma_half_finish_handler(void *param)
{
	// to do
}

void h264_dma_finish_handler(void *param)
{
	// to do 
}
void h264_dma_memcpy_handle(dma_id_t id)
{
	// to do
}

static void h264_dma_set_config(void)
{
	h264_dma_config->rx_buf = h264_dest;
	h264_dma_config->rx_buf_len = 0x10000;
	h264_dma_config->dma_channel = 0;
	h264_dma_config->transfer_len = 0x10000;
	h264_dma_config->dma_rx_half_handler = (dma_isr_t)h264_dma_half_finish_handler;
	h264_dma_config->dma_rx_finish_handler = (dma_isr_t)h264_dma_finish_handler;
	os_printf("line %d \r\n",__LINE__);
}
...
static bk_err_t h264_dma_memcpy_init(void *in, const void *out ,uint32_t len)
{
	...
	os_printf("dma id is : %d \r\n", h264_dma_memcpy_id);
	BK_LOG_ON_ERR(bk_dma_init(h264_dma_memcpy_id, &dma_config));
	BK_LOG_ON_ERR(bk_dma_enable_finish_interrupt(h264_dma_memcpy_id));
	BK_LOG_ON_ERR(bk_dma_register_isr(h264_dma_memcpy_id, NULL, h264_dma_memcpy_handle));
	BK_LOG_ON_ERR(bk_dma_set_transfer_len(h264_dma_memcpy_id, len));
#if CONFIG_SPE
	BK_LOG_ON_ERR(bk_dma_set_dest_sec_attr(h264_dma_memcpy_id, DMA_ATTR_SEC));
	BK_LOG_ON_ERR(bk_dma_set_src_sec_attr(h264_dma_memcpy_id, DMA_ATTR_SEC));
#endif
	BK_LOG_ON_ERR(bk_dma_start(h264_dma_memcpy_id));
...
static void h264_demo_main(char *pcWriteBuffer, int xWriteBufferLen, int argc, char **argv)
{
	...
	if (os_strcmp(argv[1], "encode") == 0)
	{
		...
		h264_dma_memcpy_id = bk_dma_alloc(DMA_DEV_DTCM);
		if(h264_dma_memcpy_id >= DMA_ID_MAX) {
			return;
		}
		...
		//frame_buffer_init();
		bk_dvp_camera_open(PPI_640X480, MEDIA_DVP_H264_LOCAL);
		//bk_h264_camera_clk_enhance();
	...
	if (os_strcmp(argv[1], "flash") == 0)
	{
		...
		for (int k = 0; k < flash_write_size; k++)
		{
			...
			if (k % 4 == 0)
			{
				bk_flash_erase_sector(0x200000 + k * 1024);
			}
			bk_flash_write_bytes(0x200000 + k * 1024, write_buf, 1024);
		}
		CLI_LOGI("h264 flash write finish! \r\n");
		CLI_LOGI("frame head is: \r\n");
		uint8_t a[8];
		bk_flash_read_bytes(0x200000, a, 8);
		for (int i = 0; i < 8; i++)
		{
			os_printf("%x ", *(a + i));
		}
