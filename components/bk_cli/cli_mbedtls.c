...
static void cli_sha_test_cmd(char *pcWriteBuffer, int xWriteBufferLen, int argc, char **argv){
    ...
    }else if(os_strcmp(argv[1], "256") == 0){
...
static const struct cli_command s_mbedtls_commands[] = {
	{"mbedtls_sha_test", "mbedtls_sha {1|256|512}", cli_sha_test_cmd},
	...
	{"mbedtls_pkcs5", "mbedtls_pkcs5 {test} {verbose} {type} {okm_size}", cli_mbedtls_pkcs5_cmd}
};
