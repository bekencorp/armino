...
bk_err_t http_header_destroy(http_header_handle_t header)
{
    bk_err_t err = http_header_clean(header);
    free(header);
    return err;
}
...
bk_err_t http_header_get(http_header_handle_t header, const char *key, char **value)
{
    http_header_item_handle_t item;
...
static bk_err_t http_header_new_item(http_header_handle_t header, const char *key, const char *value)
{
    http_header_item_handle_t item;

    item = calloc(1, sizeof(http_header_item_t));
    HTTP_MEM_CHECK(TAG, item, return BK_ERR_NO_MEM);
...
bk_err_t http_header_set(http_header_handle_t header, const char *key, const char *value)
{
    http_header_item_handle_t item;
...
bk_err_t http_header_set_from_string(http_header_handle_t header, const char *key_value_data)
{
    char *eq_ch;
    char *p_str;

    p_str = strdup(key_value_data);
    HTTP_MEM_CHECK(TAG, p_str, return BK_ERR_NO_MEM);
    eq_ch = strchr(p_str, ':');
    if (eq_ch == NULL) {
        free(p_str);
        return -1;
...
bk_err_t http_header_delete(http_header_handle_t header, const char *key)
{
...
int http_header_generate_string(http_header_handle_t header, int index, char *buffer, int *buffer_len)
{
    ...
    STAILQ_FOREACH(item, header, next) {
        ...
        if (siz + 1 > *buffer_len - 2) {
            ...
            ret_idx = idx - 1;
            BK_LOGE(TAG, "Buffer length is small to fit all the headers");
            break;
        }
    }
...
bk_err_t http_header_clean(http_header_handle_t header)
{
