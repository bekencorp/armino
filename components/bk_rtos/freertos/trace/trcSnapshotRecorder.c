...
static void prvTraceISRInit(void);
...
static TraceISRHandle_t s_isr_handle[INT_ID_MAX] = {0};
static void prvTraceISRInit(void)
{
	char name[16] = {0};
	for (int i = 0; i < INT_ID_MAX; i++) {
		os_memset(name, 0x0, sizeof(name));
		os_snprintf(name, sizeof(name), "isr%d", i);
		s_isr_handle[i] = xTraceSetISRProperties(name, 1);
	}
}

TraceISRHandle_t xGetTraceISRHandle(int isr_id) {
	if (isr_id < INT_ID_MAX) {
		return s_isr_handle[isr_id];
	} else {
		return s_isr_handle[0];
	}
}
