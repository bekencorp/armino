...
static int _bk_lfs_opendir(DIR *dir, const char *pathname) {
	lfs_t *lfs;
	lfs_dir_t *lfs_dir;
	int ret;
...
static int _bk_lfs_readdir(DIR *dir, struct dirent *entry) {
	lfs_t *lfs;
	lfs_dir_t *lfs_dir;
	struct lfs_info info;
	int ret;
	...
	strncpy(entry->d_name, info.name, len);

	return 0;
}

static int _bk_lfs_closedir(DIR *dir) {
	lfs_t *lfs;
	lfs_dir_t *lfs_dir;
	int ret;
