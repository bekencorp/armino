...
#define DEMO_H264_TCP_RCV_BUF_LEN               (1460)//1460
...
beken_thread_t demo_h264_tcp_task = NULL;

int demo_h264_watch_fd_list[DEMO_H264_TCP_LISTEN_MAX];
int demo_h264_tcp_server_fd;
volatile int demo_h264_tcp_running = 0;
...
int demo_h264_tcp_video_send_packet(uint8_t *data, uint32_t len)
{
	int i = 0, snd_len = 0;

	if ((!demo_h264_tcp_task) || (demo_h264_tcp_server_fd == -1))
	{
		return 0;
	}
	...
	for (i = 0; i < DEMO_H264_TCP_LISTEN_MAX; i++)
	{
		if (demo_h264_watch_fd_list[i] == -1)
		{
			continue;
		}

		rtos_lock_mutex(&send_lock);
		snd_len = write(demo_h264_watch_fd_list[i], video_tcp_send_buffer, TCP_HEAD_SIZE + len);
...
int demo_h264_tcp_audio_send_packet(unsigned char *data, unsigned int len)
{
	int i = 0, snd_len = 0;
	...
	for (i = 0; i < DEMO_H264_TCP_LISTEN_MAX; i++)
	{
		...
		rtos_lock_mutex(&send_lock);
		snd_len = write(demo_h264_watch_fd_list[i], audio_tcp_send_buffer, TCP_HEAD_SIZE + len);
		rtos_unlock_mutex(&send_lock);
		if (snd_len < 0)
		{
			/* err */
...
void demo_h264_dump(uint8_t *data, uint32_t length)
{
...
static void demo_h264_tcp_camera_data_handle(uint8_t *data, uint16_t length)
{
	if (data[0] == H264_CMD_IMG_HEADER)
	{
		uint32_t ppi = PPI_DEFAULT;
		uint8_t fmt = H264_IMG_FMT_H264;
...
static void demo_h264_tcp_cmd_data_handle(uint8_t *data, uint16_t length)
{
#if AUDIO_TRANSFER_ENABLE
	...
	LOGI("h264 cmd: %08X, param: %d, len: %d\n", cmd, param, length);
	...
	if (length >= 8)
	{
		switch (cmd)
		{
			...
			case AUDIO_OPEN:
				...
				aud_intf_drv_setup.aud_intf_tx_mic_data = demo_h264_tcp_audio_send_packet;
...
static void demo_h264_tcp_voice_data_handle(uint8_t *data, uint32_t len)
{
	LOGD("demo_h264_tcp_voice_receiver, data len=%d\n", len);
#if AUDIO_TRANSFER_ENABLE
	bk_err_t ret = BK_OK;
...
static void demo_h264_tcp_channel_data_handle(uint16_t sequence, uint16_t channel, uint8_t *data, uint16_t length)
{
	...
	switch (channel)
	{
		...
		{
			demo_h264_tcp_cmd_data_handle(data, length);
		}
		break;

		case TCP_CHANNEL_CAMERA:
		{
			demo_h264_tcp_camera_data_handle(data, length);
		}
		break;

		case TCP_CHANNEL_AUDIO:
		{
			demo_h264_tcp_voice_data_handle(data, length);
		}
		break;
...
void demo_h264_tcp_data_recv_handle(uint8_t *data, uint32_t length)
{
	static uint32_t count = 0;

	LOGD("DUMP DATA %u, size: %u\n", count++, length);

	//demo_h264_dump(data, length);
	...
	while (left != 0)
	{
		if (tcp_cache_count == 0)
		{
			...
			demo_h264_tcp_channel_data_handle(sequence, channel_id, ptr + TCP_HEAD_SIZE, channel_len);
		...
		else
		{
			...
			else
			{
				...
				demo_h264_dump(tcp_cache_buffer, tcp_cache_count);
				//TODO FIXME
				break;
			}
			...
			if (tcp_cache_count < TCP_HEAD_SIZE + channel_len)
			{
				...
				demo_h264_tcp_channel_data_handle(sequence, channel_id, tcp_cache_buffer + TCP_HEAD_SIZE, channel_len);
...
static void demo_h264_tcp_main(beken_thread_arg_t data)
{
	...
	rcv_buf = (u8 *) os_malloc((DEMO_H264_TCP_RCV_BUF_LEN + 1) * sizeof(u8));
	if (!rcv_buf)
	{
	...
	demo_h264_tcp_server_fd = socket(AF_INET, SOCK_STREAM, 0);
	if (demo_h264_tcp_server_fd == -1)
	{
		LOGE("socket failed\n");
		goto out;
	}
	...
	server_addr.sin_port = htons(DEMO_H264_TCP_SERVER_PORT);
	server_addr.sin_addr.s_addr = htonl(INADDR_ANY);

	srvaddr_len = (socklen_t)sizeof(server_addr);
	if (bind(demo_h264_tcp_server_fd, (struct sockaddr *)&server_addr, srvaddr_len) == -1)
	{
		LOGE("bind failed\n");
		goto out;
	}

	if (listen(demo_h264_tcp_server_fd, DEMO_H264_TCP_LISTEN_MAX) == -1)
	{
		LOGE("listen failed\n");
		goto out;
	}

	maxfd = demo_h264_tcp_server_fd;

	for (i = 0; i < DEMO_H264_TCP_LISTEN_MAX; i++)
	{
		demo_h264_watch_fd_list[i] = -1;
	}

	GLOBAL_INT_DISABLE();
	demo_h264_tcp_running = 1;
	GLOBAL_INT_RESTORE();

	LOGI("%s start\n", __func__);

	while (demo_h264_tcp_running)
	{
		LOGD("set fd\n");

		FD_ZERO(&watchfd);
		FD_SET(demo_h264_tcp_server_fd, &watchfd);

		for (i = 0; i < DEMO_H264_TCP_LISTEN_MAX; i++)
		{
			if (demo_h264_watch_fd_list[i] != -1)
			{
				FD_SET(demo_h264_watch_fd_list[i], &watchfd);
				if (maxfd < demo_h264_watch_fd_list[i])
				{
					maxfd = demo_h264_watch_fd_list[i];
				}
			}
		}
		...
		{
			// is new connection
			if (FD_ISSET(demo_h264_tcp_server_fd, &watchfd))
			{
				...
				new_cli_sockfd = accept(demo_h264_tcp_server_fd, (struct sockaddr *)&client_addr, &cliaddr_len);
				if (new_cli_sockfd < 0)
				{
				...
				for (i = 0; i < DEMO_H264_TCP_LISTEN_MAX; i++)
				{
					if (demo_h264_watch_fd_list[i] == -1)
					{
						demo_h264_watch_fd_list[i] = new_cli_sockfd;

						demo_h264_tcp_set_keepalive(new_cli_sockfd);
				...
				if (i == DEMO_H264_TCP_LISTEN_MAX)
				{
					LOGW("only accept %d clients\n", DEMO_H264_TCP_LISTEN_MAX);
			...
			for (i = 0; i < DEMO_H264_TCP_LISTEN_MAX; i++)
			{
				...
				if (!FD_ISSET(demo_h264_watch_fd_list[i], &watchfd))
				{
					continue;
				}

				rcv_len = recv(demo_h264_watch_fd_list[i], rcv_buf, DEMO_H264_TCP_RCV_BUF_LEN, 0);

				FD_CLR(demo_h264_watch_fd_list[i], &watchfd);

				if (rcv_len <= 0)
				{
					// close this socket
					LOGI("recv close fd:%d, rcv_len:%d\n", demo_h264_watch_fd_list[i], rcv_len);
					close(demo_h264_watch_fd_list[i]);
					demo_h264_watch_fd_list[i] = -1;
				}
				else
				{
					//LOGI("read count: %d\n", rcv_len);
					demo_h264_tcp_data_recv_handle(rcv_buf, rcv_len);
				}
...
out:
	...
	if (demo_h264_tcp_server_fd != -1)
	{
		close(demo_h264_tcp_server_fd);
		demo_h264_tcp_server_fd = -1;
	}
	...
	GLOBAL_INT_DISABLE();
	demo_h264_tcp_running = 0;
	GLOBAL_INT_RESTORE();

	demo_h264_tcp_task = NULL;
...
bk_err_t demo_H264_tcp_init(void)
{
	...
	if (!demo_h264_tcp_task)
	{
		ret = rtos_create_thread(&demo_h264_tcp_task,
		                         4,
		                         "demo_h264_tcp",
		                         (beken_thread_function_t)demo_h264_tcp_main,
		                         1024 * 6,
...
void demo_H264_tcp_deinit(void)
{
	GLOBAL_INT_DECLARATION();

	LOGI("app_demo_tcp_deinit\n");

	if (demo_h264_tcp_running == 0)
	{
		return;
	}
