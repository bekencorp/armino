...
int demo_H264_udp_img_fd = -1;
volatile int demo_H264_udp_romote_connected = 0;
volatile int demo_H264_udp_run = 0;
beken_thread_t demo_H264_udp_hdl = NULL;
struct sockaddr_in *demo_H264_remote = NULL;
int demo_H264_udp_voice_romote_connected = 0;

struct sockaddr_in *demo_H264_voice_udp_remote = NULL;

int demo_H264_udp_voice_fd = -1;
...
int demo_H264_udp_send_packet(uint8_t *data, uint32_t len)
{
	...
	if (!demo_H264_udp_romote_connected)
	{
		return 0;
	}

	send_byte = sendto(demo_H264_udp_img_fd, data, len, MSG_DONTWAIT | MSG_MORE,
	                   (struct sockaddr *)demo_H264_remote, sizeof(struct sockaddr_in));
...
#if DEMO_H264_EN_VOICE_TRANSFER
int demo_H264_udp_voice_send_packet(unsigned char *data, unsigned int len)
{
	int send_byte = 0;

	if (!demo_H264_udp_voice_romote_connected)
	{
		return 0;
	}

	send_byte = sendto(demo_H264_udp_voice_fd, data, len, MSG_DONTWAIT | MSG_MORE,
	                   (struct sockaddr *)demo_H264_voice_udp_remote, sizeof(struct sockaddr_in));
...
#endif //DEMO_H264_EN_VOICE_TRANSFER

void demo_H264_add_pkt_header(video_packet_t *param)
{
...
static void demo_H264_udp_handle_cmd_data(uint8_t *data, UINT16 len)
{
	...
	if (len >= 8)
	{
		...
		switch (cmd)
		{
			...
			case AUDIO_OPEN:
				...
				aud_intf_drv_setup.aud_intf_tx_mic_data = demo_H264_udp_voice_send_packet;
...
static void demo_H264_udp_app_connected(void)
{
	//app_demo_softap_send_msg(DAP_APP_CONECTED, 0);
}

static void demo_H264_udp_app_disconnected(void)
{
	//app_demo_softap_send_msg(DAP_APP_DISCONECTED, 0);
}


static void demo_H264_udp_receiver(uint8_t *data, uint32_t len, struct sockaddr_in *demo_H264_remote)
{
	LOGD("demo_H264_udp_receiver\n");

	GLOBAL_INT_DECLARATION();

	uint32_t ppi = PPI_DEFAULT;
	uint8_t fmt = H264_IMG_FMT_MJPEG;
	...
	if (data[0] == H264_CMD_IMG_HEADER)
	{
		...
		switch (data[1])
		{
			case H264_DVP_START:
			{
				...
				uint8_t *src_ipaddr = (uint8_t *)&demo_H264_remote->sin_addr.s_addr;
				LOGI("src_ipaddr: %d.%d.%d.%d\n", src_ipaddr[0], src_ipaddr[1],
				     src_ipaddr[2], src_ipaddr[3]);
				LOGI("udp connect to new port:%d\n", demo_H264_remote->sin_port);

				GLOBAL_INT_DISABLE();
				demo_H264_udp_romote_connected = 1;
				GLOBAL_INT_RESTORE();
				...
				setup.send_func = demo_H264_udp_send_packet;
				setup.start_cb = demo_H264_udp_app_connected;
				setup.end_cb = demo_H264_udp_app_disconnected;
			...
			{
				GLOBAL_INT_DISABLE();
				demo_H264_udp_romote_connected = 0;
				GLOBAL_INT_RESTORE();
...
#if DEMO_H264_EN_VOICE_TRANSFER
static void demo_H264_udp_voice_receiver(uint8_t *data, uint32_t len, struct sockaddr_in *udp_remote)
{
	LOGD("demo_H264_udp_voice_receiver, data len=%d\n", len);
#if AUDIO_TRANSFER_ENABLE
	bk_err_t ret = BK_OK;

	if (len > 0)
	{
		demo_H264_udp_voice_romote_connected = 1;
	}
...
static void demo_H264_udp_main(beken_thread_arg_t data)
{
	GLOBAL_INT_DECLARATION();
	...
	demo_H264_remote = (struct sockaddr_in *)os_malloc(sizeof(struct sockaddr_in));
	if (!demo_H264_remote)
	{
		LOGE("udp os_malloc failed\n");
		goto out;
	...
	// for data transfer
	demo_H264_udp_img_fd = socket(AF_INET, SOCK_DGRAM, 0);
	if (demo_H264_udp_img_fd == -1)
	{
		LOGE("socket failed\n");
		goto out;
	}

	demo_H264_remote->sin_family = AF_INET;
	demo_H264_remote->sin_port = htons(DEMO_H264_UDP_IMG_PORT);
	demo_H264_remote->sin_addr.s_addr = htonl(INADDR_ANY);

	srvaddr_len = (socklen_t)sizeof(struct sockaddr_in);
	if (bind(demo_H264_udp_img_fd, (struct sockaddr *)demo_H264_remote, srvaddr_len) == -1)
	{
		LOGE("bind failed\n");
		goto out;
	}
	...
	cmd_remote.sin_port = htons(DEMO_H264_UDP_CMD_PORT);
	...
	maxfd = (udp_cmd_fd > demo_H264_udp_img_fd) ? udp_cmd_fd : demo_H264_udp_img_fd;

	// for voice transfer
#if DEMO_H264_EN_VOICE_TRANSFER
	demo_H264_voice_udp_remote = (struct sockaddr_in *)os_malloc(sizeof(struct sockaddr_in));
	if (!demo_H264_voice_udp_remote)
	{
		LOGE("udp os_malloc failed\n");
		goto out;
	}

	demo_H264_udp_voice_fd = socket(AF_INET, SOCK_DGRAM, 0);
	if (demo_H264_udp_voice_fd == -1)
	{
		LOGE("vo socket failed\n");
		goto out;
	}

	demo_H264_voice_udp_remote->sin_family = AF_INET;
	demo_H264_voice_udp_remote->sin_port = htons(DEMO_H264_UDP_VOICE_PORT);
	demo_H264_voice_udp_remote->sin_addr.s_addr = htonl(INADDR_ANY);

	srvaddr_len = (socklen_t)sizeof(struct sockaddr_in);
	if (bind(demo_H264_udp_voice_fd, (struct sockaddr *)demo_H264_voice_udp_remote, srvaddr_len) == -1)
	{
		LOGE("bind failed\n");
		goto out;
	}
	maxfd = (maxfd > demo_H264_udp_voice_fd) ? maxfd : demo_H264_udp_voice_fd;
#endif // DEMO_H264_EN_VOICE_TRANSFER
	...
	demo_H264_udp_run = 1;
#if DEMO_H264_EN_VOICE_TRANSFER
	demo_H264_udp_voice_romote_connected = 0;
#endif
	GLOBAL_INT_RESTORE();

	while (demo_H264_udp_run)
	{
		FD_ZERO(&watchfd);
		FD_SET(demo_H264_udp_img_fd, &watchfd);
		FD_SET(udp_cmd_fd, &watchfd);
#if DEMO_H264_EN_VOICE_TRANSFER
		FD_SET(demo_H264_udp_voice_fd, &watchfd);
#endif
		...
		else
		{
			...
			if (FD_ISSET(demo_H264_udp_img_fd, &watchfd))
			{
				rcv_len = recvfrom(demo_H264_udp_img_fd, rcv_buf, APP_DEMO_UDP_RCV_BUF_LEN, 0,
				                   (struct sockaddr *)demo_H264_remote, &srvaddr_len);

				if (rcv_len <= 0)
				{
					// close this socket
					LOGE("recv close fd:%d\n", demo_H264_udp_img_fd);
					break;
				}
				else
				{
					...
					LOGE("demo_H264_udp_receiver\n");
					demo_H264_udp_receiver(rcv_buf, rcv_len, demo_H264_remote);
				}
			}
			else if (FD_ISSET(udp_cmd_fd, &watchfd))
			{
				...
				else
				{
					...
					demo_H264_udp_handle_cmd_data(rcv_buf, rcv_len);
				}
			}
#if DEMO_H264_EN_VOICE_TRANSFER
			if (FD_ISSET(demo_H264_udp_voice_fd, &watchfd))
			{
				rcv_len = recvfrom(demo_H264_udp_voice_fd, rcv_buf, APP_DEMO_UDP_RCV_BUF_LEN, 0,
				                   (struct sockaddr *)demo_H264_voice_udp_remote, &srvaddr_len);

				if (rcv_len <= 0)
				{
					// close this socket
					LOGE("recv close fd:%d\n", demo_H264_udp_voice_fd);
					break;
				}
				else
				{
					rcv_len = (rcv_len > APP_DEMO_UDP_RCV_BUF_LEN) ? APP_DEMO_UDP_RCV_BUF_LEN : rcv_len;
					rcv_buf[rcv_len] = 0;
					demo_H264_udp_voice_receiver(rcv_buf, rcv_len, demo_H264_voice_udp_remote);
				}
			}
#endif // DEMO_H264_EN_VOICE_TRANSFER
		}
...
out:
	...
	LOGE("demo_H264_udp_main exit %d\n", demo_H264_udp_run);
	media_app_transfer_close();
	...
	if (demo_H264_udp_voice_fd != -1)
	{
		close(demo_H264_udp_voice_fd);
		demo_H264_udp_voice_fd = -1;
	}

	if (demo_H264_voice_udp_remote)
	{
		os_free(demo_H264_voice_udp_remote);
		demo_H264_voice_udp_remote = NULL;
	}
#endif // DEMO_H264_EN_VOICE_TRANSFER
	if (demo_H264_remote)
	{
		os_free(demo_H264_remote);
		demo_H264_remote = NULL;
	}

	if (demo_H264_udp_img_fd != -1)
	{
		close(demo_H264_udp_img_fd);
		demo_H264_udp_img_fd = -1;
	}
...
bk_err_t demo_H264_udp_init(void)
{
	...
	if (!demo_H264_udp_hdl)
	{
		ret = rtos_create_thread(&demo_H264_udp_hdl,
		                         4,
		                         "H264",
		                         (beken_thread_function_t)demo_H264_udp_main,
		                         1024 * 2,
