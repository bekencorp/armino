...
static void tvideo_rx_handler(void *curptr, uint32_t newlen, uint32_t is_eof, uint32_t frame_len) {
	bk_printf("curptr: %p, newlen: %d, is end of frame: %d, frame_len\r\n", curptr, newlen, is_eof, frame_len);
}

static void tvideo_end_frame_handler(void)
{
	bk_printf("end of frame\r\n");
}

video_config_t* bk_camera_config_init(void) {
	video_config_t *cfg = (video_config_t *)os_malloc(sizeof(video_config_t));
	if (NULL == cfg) {
		return NULL;
	}
	os_memset(cfg, 0, sizeof(video_config_t));

	cfg->rxbuf_len = 8 * 1024;
	cfg->rxbuf = os_malloc(cfg->rxbuf_len);
	if (NULL == cfg->rxbuf) {
		os_free(cfg);
		return NULL;
	}
	os_memset(cfg->rxbuf, 0, cfg->rxbuf_len);

	cfg->node_len = 2048;
	cfg->rx_read_len = 0;
	cfg->sener_cfg = 0;

	CMPARAM_SET_PPI(tvideo_st.sener_cfg, VGA_480_320;
	CMPARAM_SET_FPS(tvideo_st.sener_cfg, TYPE_5FPS);

	cfg->node_full_handler = tvideo_rx_handler;
	cfg->data_end_handler = tvideo_end_frame_handler;
	return cfg;
}
