...
void cli_aud_adc_to_file_test_cmd(char *pcWriteBuffer, int xWriteBufferLen, int argc, char **argv)
{
	bk_err_t ret = BK_OK;
	aud_adc_config_t adc_config;
	dma_config_t dma_config;
	...
	if (os_strcmp(argv[1], "start") == 0) {
		os_printf("audio adc to file test start\n");
		adc_test_mode = ADC_TEST_MODE_MCP;
		adc_config.samp_rate = AUD_ADC_SAMP_RATE_8K;
		adc_config.adc_enable = AUD_ADC_DISABLE;
		adc_config.line_enable = AUD_ADC_LINE_DISABLE;
		...
		os_printf("start audio adc to file test successful\r\n");
	} else if (os_strcmp(argv[1], "stop") == 0) {
...
void cli_aud_adc_to_sd_test_cmd(char *pcWriteBuffer, int xWriteBufferLen, int argc, char **argv)
{
	bk_err_t ret = BK_OK;
	aud_adc_config_t adc_config;
	dma_config_t dma_config;
