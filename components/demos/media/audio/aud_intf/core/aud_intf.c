...
bk_err_t bk_aud_intf_set_mic_gain(uint8_t value)
{
	...
	if (aud_intf_info.mic_status != AUD_INTF_MIC_STA_NULL || aud_intf_info.voc_status != AUD_INTF_VOC_STA_NULL) {
		if (aud_intf_info.mic_status != AUD_INTF_MIC_STA_NULL) {
			...
			return aud_tras_drv_send_msg(AUD_TRAS_DRV_MIC_SET_GAIN, &aud_intf_info.mic_info.mic_gain);
		}

		if (aud_intf_info.voc_status != AUD_INTF_VOC_STA_NULL) {
			aud_intf_info.voc_info.aud_setup.adc_gain = value;
			return aud_tras_drv_send_msg(AUD_TRAS_DRV_VOC_SET_MIC_GAIN, &aud_intf_info.voc_info.aud_setup.adc_gain);
		}
	}
...
{
	return aud_tras_drv_send_msg(AUD_TRAS_DRV_MIC_DEINIT, NULL);
}

bk_err_t bk_aud_intf_mic_start(void)
{
	return aud_tras_drv_send_msg(AUD_TRAS_DRV_MIC_START, NULL);
}

bk_err_t bk_aud_intf_mic_pause(void)
{
	return aud_tras_drv_send_msg(AUD_TRAS_DRV_MIC_PAUSE, NULL);
}

bk_err_t bk_aud_intf_mic_stop(void)
{
	return aud_tras_drv_send_msg(AUD_TRAS_DRV_MIC_STOP, NULL);
}

static void aud_intf_spk_deconfig(void)
{
	aud_intf_info.spk_info.spk_chl = AUD_INTF_SPK_CHL_SPK1;
...
static void aud_tras_drv_spk_event_cb_handle(aud_tras_drv_spk_event_t event, void *param)
{
	switch (event) {
...
{
	return aud_tras_drv_send_msg(AUD_TRAS_DRV_SPK_DEINIT, NULL);
}

bk_err_t bk_aud_intf_spk_start(void)
{
	return aud_tras_drv_send_msg(AUD_TRAS_DRV_SPK_START, NULL);
}

bk_err_t bk_aud_intf_spk_pause(void)
{
	return aud_tras_drv_send_msg(AUD_TRAS_DRV_SPK_PAUSE, NULL);
}

bk_err_t bk_aud_intf_spk_stop(void)
{
	return aud_tras_drv_send_msg(AUD_TRAS_DRV_SPK_STOP, NULL);
}
...
static void aud_tras_drv_voc_event_cb_handle(aud_tras_drv_voc_event_t event, void *param)
{
	switch (event) {
...
bk_err_t bk_aud_intf_voc_deinit(void)
{
	...
	return aud_tras_drv_send_msg(AUD_TRAS_DRV_VOC_DEINIT, NULL);
}

bk_err_t bk_aud_intf_voc_start(void)
{
	return aud_tras_drv_send_msg(AUD_TRAS_DRV_VOC_START, NULL);
}

bk_err_t bk_aud_intf_voc_stop(void)
{
	return aud_tras_drv_send_msg(AUD_TRAS_DRV_VOC_STOP, NULL);
}
...
bk_err_t bk_aud_intf_write_spk_data(uint8_t *dac_buff, uint32_t size)
{
	...
	switch (aud_intf_info.drv_info.work_mode) {
