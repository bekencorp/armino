...
void image_save_dvp(char *pcWriteBuffer, int xWriteBufferLen, int argc, char **argv)
{
	...
	if (os_strcmp(argv[1], "init") == 0) {
		...
		device.camera_type = MEDIA_DVP_MJPEG;
#if (CONFIG_PSRAM)
		err = bk_psram_init();
		if (err != kNoErr) {
		...
		if (argc == 5)
		{
			device.camera_type = os_strtoul(argv[2], NULL, 10);
			device.ppi = get_string_to_ppi(argv[3]);
			device.fps = get_string_to_fps(argv[4]);
		}

		if (device.camera_type >= MEDIA_CAMERA_UNKNOW)
		{
			os_printf("config param error\r\n");
			goto error;
		}
