...
static void* alloc_pool (	/* Pointer to allocated memory block (NULL:no memory available) */
	jpegdec_config_t* jd,		/* Pointer to the decompressor object */
		...
		{		/* Next byte? */
			...
			{	/* No input data is available, re-fill input buffer */
				...
				if (!dc)
				{
					...
					return 0 - (int16_t)JDR_INP;	/* Err: read error or wrong stream termination */
				}
			} 
			else 
			{
...
static JRESULT mcu_output (
	jpegdec_config_t* jd,		/* Pointer to the decompressor object */
	uint16_t (*outfunc)(jpegdec_config_t*, void*, JRECT*),	/* RGB output function */
...
)
{
	...
	{	/* Not for 1/8 scaling */
		...
		if (JD_FORMAT < 2) {
			...
			for (iy = 0; iy < my; iy++) {
				...
				for (ix = 0; ix < mx; ix++) {
					...
					*rgb24++ = (uint8_t)*py++;			/* Get and store a Y value as grayscale */
				}
			}
		} else { /* JD_FORMAT == 3, yuv422(yyuv) output*/
			for (iy = 0; iy < my; iy++) {
				pc = py = jd->mcubuf;
				if (my == 16) {		/* Double block height? */
					pc += 64 * 4 + (iy >> 1) * 8;
				...
				py += iy * 8;
				for (ix = 0; ix < mx; ix++) {
					cb = pc[0]; 	/* Get Cb/Cr component and remove offset */
					cr = pc[64];
					if (mx == 16) {					/* Double block width? */
						if (ix == 8) 
		...
		if (JD_USE_SCALE && jd->scale) {
			...
			/* Get averaged RGB value of each aquare correcponds to a pixel*/
			s = jd->scale * 2; /* Number of shifts for averging */
			w = 1 << jd->scale; /* Width of square*/
			a = (mx - w) * (JD_FORMAT < 2 ? 3 : 1); /*Byte to skip for next line tn the square */
			op = (uint8_t*)jd->workbuf;
			for (iy = 0; iy < my; iy += w) {
				for (ix = 0; ix < mx; ix += w) {
					rgb24 = (uint8_t*)jd->workbuf + (iy * mx + ix) * (JD_FORMAT < 2 ? 3 : 1);
					r = g = b = 0;
					for (y = 0; y < w; y++) {	/* Accumulate RGB value in the square */
						for (x = 0; x < w; x++) {
							r += *rgb24++;	/* Accumulate R or Y (monochrome output) */
							if (JD_FORMAT < 2) {	/* RGB output? */
								g += *rgb24++;	/* Accumulate G */
								b += *rgb24++;	/* Accumulate B */
							}
						}
						rgb24 += a;
...
JRESULT jpeg_decode_prepare (
	jpegdec_config_t* jd,			/* Blank decompressor object */
	uint16_t (*infunc)(jpegdec_config_t*, uint8_t*, uint16_t),	/* JPEG strem input function */
...
)
{
	...
	if (LDB_WORD(seg) != 0xFFD8)  {
		os_printf("/* Err: SOI is not detected %d*/\r\n", __LINE__);
	...
	for (;;) {
		...
		/*skip the first maker(0xff),treat the following 0xff as the marker*/
		case 0xFF:
			if(0xff != seg[2])
			{
				;
...
/*-----------------------------------------------------------------------*/
JRESULT jpeg_decode_comp (
	jpegdec_config_t* jd,								/* Initialized decompression object */
	uint16_t (*outfunc)(jpegdec_config_t*, void*, JRECT*),	/* RGB output function */
