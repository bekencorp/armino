	...
	while (ndata) {	/* Process all tables in the segment */
		...
		{	/* Create fast huffman decode table */
			...
			if (cls) {
				...
				os_memset(tbl_ac, 0xFF, HUFF_LEN * sizeof (uint16_t));		/* Default value (0xFFFF: may be long code) */
			} else {
				...
				os_memset(tbl_dc, 0xFF, HUFF_LEN * sizeof (uint8_t));		/* Default value (0xFF: may be long code) */
			}
...
)
{
	...
	for (blk = 0; blk < nby + 2; blk++) {	/* Get nby Y blocks and two C blocks */
		...
		if (cmp && jd->ncomp != 3) {		/* Clear C blocks if not exist (monochrome image) */
			...
			os_memset(&tmp[1], 0, 63 * sizeof (int32_t));	/* Initialize all AC elements */
...
)
{
	...
	os_memset(jd, 0, sizeof (JDEC));	/* Clear decompression object (this might be a problem if machine's null pointer is not all bits zero) */
