...
int dhcp_server_init(void *intrfc_handle)
{
	...
	//Bind AP's IP for src-ip-based routing
	dhcps.saddr.sin_addr.s_addr = dhcps.my_ip;
	...
	if (dhcp_nack_dns_server_handler) {
		...
		//Bind AP's IP for src-ip-based routing
		dhcps.dnsaddr.sin_addr.s_addr = dhcps.my_ip;
...
{
#if BK_DHCPS_DNS
	/* DHCP server should NOT processing any DNS message, it should be handled in LWIP dns module if any.*/
