...
void xPortDumpMemStats(uint32_t start_tick, uint32_t ticks_since_malloc, const char* task)
{
	...
	BK_LOGI(TAG, "%-8s   %-10s   %-4s   %-5s", "tick", "addr", "size", "line");
	...
	list_for_each_entry(pxLink, &xUsed, node) {
		...
		BK_LOGI(TAG, "%-8d   0x%-8x   %-4d   %-5d", pxLink->allocTime,
		(u8*)pxLink + xHeapStructSize, pxLink->wantedSize, pxLink->line);
#if CONFIG_MEM_DEBUG_FUNC_NAME
		BK_LOG_RAW("   %-16s", pxLink->funcName);
#endif

#if CONFIG_MEM_DEBUG_TASK_NAME
		BK_LOG_RAW("   %-16s", pxLink->taskName);
#endif
		BK_LOG_RAW("\r\n");
	}
