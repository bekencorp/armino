...
#define TAG "queue"
	...
	#define queueGET_ISR_STATUS()
#else

#if( configDEBUG_YIELD == 1)
	#define queueGET_ISR_STATUS() uint32_t isr_disabled = platform_is_irq_disable()

	#define queueYIELD_IF_USING_PREEMPTION() do {\
			if (isr_disabled) {\
				BK_LOGW(TAG, "%s %d yield after ISR disabled!\n", __FUNCTION__, __LINE__);\
			}\
