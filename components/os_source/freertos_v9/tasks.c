 ...
 */
	...
	static UBaseType_t prvTaskCheckFreeStackSpace( const uint8_t * pucStackByte ) PRIVILEGED_FUNCTION;
...
/*-----------------------------------------------------------*/
	...
	BaseType_t xReturn;
		...
		{
			#if configBK_FREERTOS
			pxNewTCB->ulStackSize = usStackDepth * sizeof( StackType_t );
			#endif
...
#if ( ( configUSE_TRACE_FACILITY == 1 ) || ( INCLUDE_uxTaskGetStackHighWaterMark == 1 ) )
	static UBaseType_t prvTaskCheckFreeStackSpace( const uint8_t * pucStackByte )
	{
	UBaseType_t ulCount = 0U;
...
static void vTaskDumpThreadListStack(List_t *list)
{
	...
	ListItem_t *item = list->xListEnd.pxNext;
