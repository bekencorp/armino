...
static int32_t is_range_valid(struct emulated_flash_dev_t *flash_dev,
                              uint32_t offset)
{
...
static int32_t is_write_aligned(struct emulated_flash_dev_t *flash_dev,
                                uint32_t param)
{
    int32_t rc = 0;
...
static int32_t is_sector_aligned(struct emulated_flash_dev_t *flash_dev,
                                 uint32_t offset)
{
    int32_t rc = 0;
...
enum emulated_flash_error_t emulated_flash_read_data(struct emulated_flash_dev_t* dev,
...
enum emulated_flash_error_t emulated_flash_program_data(struct emulated_flash_dev_t* dev,
...
enum emulated_flash_error_t emulated_flash_erase_sector(struct emulated_flash_dev_t* dev,
                                                        uint32_t addr)
{
    uint32_t start_addr = dev->memory_base + addr;
    uint32_t rc = 0;
    ...
    memset((void *)start_addr,
           dev->data->erased_value,
           dev->data->sector_size);
    return EMULATED_FLASH_ERR_NONE;
}
