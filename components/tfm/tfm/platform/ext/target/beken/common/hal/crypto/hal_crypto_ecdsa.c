                                         ...
                                         size_t sig_s_size)
{
    ...
    /* load group */
    ret = mbedtls_ecp_group_load(&(ecdsa_ctx.private_grp), grp_id);
    HAL_CHECK_CONDITION(0 == ret,
                        HAL_ERR_INTERNAL_FATAL,
    ...
    key_size = (ecdsa_ctx.private_grp.pbits + 7) / 8;
    ...
    ret = mbedtls_mpi_read_binary(&(ecdsa_ctx.private_Q.private_X), key->ec_pubkey.q + 1, key_size);
    HAL_CHECK_CONDITION(0 == ret,
                        HAL_ERR_INTERNAL_FATAL,
                        "Call mbedtls_mpi_read_binary failed: -0x%x\n",
                        -ret);

    ret = mbedtls_mpi_read_binary(&(ecdsa_ctx.private_Q.private_Y), key->ec_pubkey.q + 1 + key_size, key_size);
    HAL_CHECK_CONDITION(0 == ret,
                        HAL_ERR_INTERNAL_FATAL,
                        "Call mbedtls_mpi_read_binary failed: -0x%x\n",
                        -ret);

    ret = mbedtls_mpi_read_binary(&(ecdsa_ctx.private_Q.private_Z), &q_z, 1);
    HAL_CHECK_CONDITION(0 == ret,
                        HAL_ERR_INTERNAL_FATAL,
                        "Call mbedtls_mpi_read_binary failed: -0x%x\n",
                        -ret);
    ...
    ret = mbedtls_ecdsa_verify(&(ecdsa_ctx.private_grp),
                               dgst,
                               dgst_size,
                               &(ecdsa_ctx.private_Q),
                               &sig_r_mpi,
