...
Example 1: Take bk7235 as an example. If the following compiling error occurs after the user adds code to the SDK, see Figure 2. What should I do?
...
.. important::
  - First of all, you can manually will middleware/soc/bk7235/bk7235_bsp. Corresponding increases the size range of ld (ld in the last few lines).
  - Solution 1: if you exceed code size is small, can shut down a few do not need to function, that is in the middleware/soc/bk7235/bk7235.defconfig close corresponding macro configuration, reduce the code size. If the excess code size is large, only Solution 2 can be implemented.
...
- 1. Making app partition table:
  ...
  The total physical length of flash is 4M, Must be in the middleware/boards/bk7235/vnd_flash/vnd_flash.c set partitions partition_start_addr (physical address) and partition_length (physical length) as shown in figure 3 yellow area;
...
- 2. Package script configuration:
  ...
  The middleware/boards/bk7235/configuration.json, The start_addr and size according to figure 3 each partition in the partition table size changes, (including start_addr = partition_start_addr,size = partition_length) see yellow area in Figure 4;
...
- 3. Link script configuration: If bk7235 is single-core, bk7235_bsp.sag needs to be configured.

The bk7235_bsp.sag configuration is as follows:

  - Middleware/soc/bk7235/bk7235_bsp, the LMA and length according to the Application in figure 3 the partition table and Application1 partition_start_addr and partition_length is calculated See the yellow area in Figure 5.
...
.. figure:: ../../../_static/app0_sag_35.png
    ...
    Figure 5 Link script configuration (bk7235_bsp.sag)

.. note::

  Calculation rules:
    - In bk7235_bsp.sag, LMA (cpu view 0x10000) of CPU0_LOAD1 = partition_start_addr \*32/34 of Application;

    - The length (cpu view 0x8800) of CPU0_LOAD1 in bk7235_bsp.sag = 0x8800;

    - In bk7235_bsp.sag, the LMA of CPU0_LOAD2 (cpu view 0x18800) = the LMA of CPU0_LOAD1 + 0x8800;
...
- 4. Use the Andes sag to convert bk7235_bsp.sag and bk7235_cp1_bsp.sag method in RISC_V to convert bK7235_cp1_bsp.sag to corresponding ld files.
...
- 5. (For dual-core, perform Step 5. For single-core, do not perform Step 5. Here does not need to) ------ should be middleware/soc/bk7235/bk7235.defconfig CPU1 nuclear offset in the reset CONFIG_SLAVE_CORE_OFFSET = 1835008 (in decimal format) (CONFIG_SLAVE_CORE_OFFSET 1835008 == 0x1C0000 in bk7235_cp1_bsp.sag LMA)
...
- 6.Use tools/env_tools/rt_partition_tool/rt_partition_tool.exe to modify the partition table of the new bootloader. See Figure 7.
  ...
  - loading middleware/boards/bk7235/bootloader.bin (if no partition table in the bootloader will prompt will add import partition table)
