...
typedef struct
{
	...
	uint8_t *buffer;
	void (*node_full_handler)(uint8_t *curptr, uint32_t len);
} uvc_camera_drv_t;
...
static void uvc_process_data_packet(uint8_t *curptr, uint32_t newlen)
{
	uint8_t *data = NULL;
...
#ifdef UVC_STRIP
	uvc_packet_check(curptr, newlen);
#endif
	...
	else
	{
		...
		// step 4: check need drop frame
		if (uvc_drop_frame)
		{
			UVC_LOGD("uvc_drop_frame:%d\r\n", uvc_drop_frame);
		...
		// step 5: check if current packet is new frame
		...
		//UVC_LOGI("%02x %02x %02x %02x %d\r\n", data[0], data[1], data[2], data[3], data_len);

		// step 6: compy payload to frame_buffer
		if (data_len > 0)
		{
...
static void uvc_bulk_transfer_mode_handle(uint8_t *arg, uint32_t count)
{
	...
	uvc_camera_dma_finish_callback(0);
}
...
static void uvc_get_packet_rx_vs_callback(uint8_t *arg, uint32_t count)
{
	...
	if (count > uvc_camera_drv->wMaxPacketSize)
	{
		uvc_camera_drv->wMaxPacketSize = count;
	}
