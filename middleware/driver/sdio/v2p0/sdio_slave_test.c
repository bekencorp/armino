...
static bk_err_t sdio_test_tx_cb(SDIO_HEADER_PTR head_p, SDIO_HEADER_PTR tail_p, uint32_t count)
{
	bk_err_t ret = BK_OK;
	SDIO_HEADER_PTR cur_p = head_p;
	...
	while(cur_p && i > 0)
	{
		...
		{
			os_printf("%x ", (uint8_t *)cur_p + (sizeof(SDIO_HEADER_T)) + i);
			if(i % 16 == 0)
				os_printf("\r\n");
		}
	...
	switch(msg_p->sub_case)
	{
		...
		case SDIO_TEST_TX_SINGLE_PACKET:
		{
			...
			if(buf_p)
			{
				...
				memset((uint8_t *)buf_p + sizeof(SDIO_HEADER_T), value, len);
				buf_p->len = len;
...
static bk_err_t sdio_test_rx_cb(SDIO_HEADER_PTR head_p, SDIO_HEADER_PTR tail_p, uint32_t count)
{
	bk_err_t ret = BK_OK;
	SDIO_HEADER_PTR cur_p = head_p;
	uint32_t i = count, j = 0;
