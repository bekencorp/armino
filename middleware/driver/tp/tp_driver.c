...
{
	uint8_t size = sizeof(tp_sensor_configs) / sizeof(tp_sensor_config_t *);

	for (uint8_t i = 0; i < size; i++)
	{
		if (tp_sensor_configs[i]->id == id)
		{
			return tp_sensor_configs[i];
		}
	}
...
const tp_sensor_config_t *tp_get_sensor_auto_detect(const tp_i2c_callback_t *cb)
{
	...
	for (uint8_t i = 0; i < size; i++)
	{
		if (NULL != tp_sensor_configs[i]->detect)
		{
			if (true == tp_sensor_configs[i]->detect(cb))
			{
				return tp_sensor_configs[i];
			}
		}
