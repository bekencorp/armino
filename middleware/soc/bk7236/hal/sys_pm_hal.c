...
static uint8_t s_lpo_src = CONFIG_DEFAULT_LPO_SRC;

static inline bool is_lpo_src_26m32k(void)
{
	return (s_lpo_src == SYS_LPO_SRC_26M32K);
}
...
static inline uint32_t sys_hal_disable_hf_clock(void)
{
	IF_LV_CTRL_HF() {
		...
		if (is_lpo_src_26m32k()) {
			PM_HAL_LOGD("not disable 26M xtal clock\r\n");
			val |= HF_XTAL;
		}
...
static inline void sys_hal_power_down_pd(volatile uint32_t *pd_reg_v)
{
	IF_LV_CTRL_PD() {
		...
		*pd_reg_v = v;
		...
		if (is_wifi_ws_enabled(ws_ena)) {
			PM_HAL_LOGD("not power down WiFi\r\n");
			v &= ~PD_WIFI;
		...
		PM_HAL_LOGD("enable lpo when halt\r\n");
		aon_pmu_ll_set_r41_halt_lpo(1);
...
void sys_hal_enter_deep_sleep(void *param)
{
	...
	aon_pmu_ll_set_r41_lpo_config(SYS_LPO_SRC_ROSC);
...
//	sys_ll_set_ana_reg8_aldohp(0);//tenglong20230417 make current 12uA higher in ldo mode
#endif
...
__attribute__((section(".iram"))) void sys_hal_enter_low_voltage(void)
{
	...
	aon_pmu_ll_set_r41_lpo_config(s_lpo_src);
...
#if 0//TODO if vedio power down, dvp open failed!
	uint32_t v = sys_ll_get_cpu_power_sleep_wakeup_value();
	v |= BIT(7);//vedio
	sys_ll_set_cpu_power_sleep_wakeup_value(v);
#endif
