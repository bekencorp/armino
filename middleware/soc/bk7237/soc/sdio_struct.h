...
typedef volatile struct {
    ...
    /* REG_0x01 */
    volatile uint32_t  cmd_argument;  //0x1[31:0],Host Only; sd host send command argument value; Host:CMD index …,0x0,RW

    /* REG_0x02 */
    volatile uint32_t  sd_cmd_timer;  //0x2[31:0],Host Only; sd host wait the slave respond timer; What's the unit?,0xffffffff,RW
    ...
    /* REG_0x04 */
    volatile uint32_t sd_data_timer;  //0x4[31:0],Host:; sd host read data from slave till timeout,; what's write timeout mean?; Slave:; slave wait host write data to line till timeout,it doesn't check host read data time.,0xffffffff,RW

    /* REG_0x05 */
    volatile uint32_t  sd_rsp_agument_0;  //0x5[31:0],Host:sd host received slave command argument value; ; Slave:sd slave received host command argument.; The argument maybe CMD52/CMD53 and other value(Though other CMD isn't error by ASIC).Every CMD Response INT should check this value.,0x0,R

    /* REG_0x06 */
    volatile uint32_t  sd_rsp_agument_1;  //0x6[31:0],Host only; sd host received slave command argument value,0x0,R

    /* REG_0x07 */
    volatile uint32_t  sd_rsp_agument_2;  //0x7[31:0],Host only; sd host received slave command argument value,0x0,R

    /* REG_0x08 */
    volatile uint32_t  sd_rsp_agument_3;  //0x8[31:0],Host only; sd host received slave command argument value,0x0,R
    ...
    { 
        struct 
        { 
           volatile uint32_t  sd_cmd_send_no_rsp_end_int      : 1;  //0x9[0],Host only; sd host send no response command end int.; Some CMD no needs response,so CMD sends finish then the sdio produced the CMD send finish INT,0x0,R/W1C
           ...
           volatile uint32_t  rx_overflow_int                          : 1;  //0x9[8],sd host fifo memory overflow int?; rx fifo full, host slave wr too many full, slave host wr too many data.; What's the overflow condition?,0x0,R/W1C
           volatile uint32_t  tx_fifo_empty_int                   : 1;  //0x9[9],sd host fifo memory empty int,0x0,R
    ...
    { 
        struct 
        { 
           volatile uint32_t  sd_cmd_send_no_rsp_end_mask     : 1;  //0xa[0],sd host send no response command end mask; Slave Invalid,0x0,RW
           ...
           volatile uint32_t  rx_overflow_mask                     : 1;  //0xa[8],sd host fifo memory overflow mask; Slave Invalid,0x0,RW
           volatile uint32_t  tx_fifo_empty_mask              : 1;  //0xa[9],sd host fifo memory empty mask：; a.0:Mask INT,not report to CPU, 1：If empty,INT report to CPU(If FIFO is empty,SDIO module will be clock gate, FIFO not empty, clock restart); b.Must after CMD53; workaround:multi-block has to set 1,0x0,RW
    ...
    /* REG_0x0b */
    volatile uint32_t  tx_fifo_din;  //0xb[31:0],sd host fifo write data,0x0,W

    /* REG_0x0c */
    volatile uint32_t  rx_fifo_dout;  //0xc[31:0],sd host fifo read data,0x0,R
    ...
    { 
        ...
        { 
           ...
           volatile uint32_t  rx_fifo_reset                    : 1;  //0xd[16],read fifo reset, high active; Host/Slave:Reset self state,0x1,R/W1C
           volatile uint32_t  tx_fifo_reset                    : 1;  //0xd[17],write fifo reset, high active; Host/Slave:Reset self state,0x1,R/W1C
           volatile uint32_t  rx_fifo_rd_ready                : 1;  //0xd[18],host/slave rx fifo not empty,0x0,R
           volatile uint32_t  tx_fifo_wr_ready                : 1;  //0xd[19],host/slave tx fifo not full(data is valid, not used for CMD),0x1,R
           volatile uint32_t  sd_start_reset                     : 1;  //0xd[20],sdcard command and data state reset, high active; Host/Slave:Reset self state; It's better to reset after one round transfer.,0x1,R/W1C
