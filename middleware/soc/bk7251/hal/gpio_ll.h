...
{
	for (int index = GPIO_2; index < GPIO_32; index++) {
		//hw->gpio_0_31_num[index].cfg.v = 0;
	}

	for (int index = GPIO_32; index <= GPIO_39; index++) {
		hw->gpio_32_39_num[index-GPIO_32].cfg.v = 0x28;
	}
}
...
static inline void gpio_ll_set_io_mode(gpio_hw_t *hw, uint32 index, const gpio_config_t *config)
{
	...
	if (index < GPIO_32)
		REG_SET_BITS(&hw->gpio_0_31_num[index], GPIO_F_IO_MODE_SET(io_mode), GPIO_F_IO_MODE_EN_M) ;
	else
		REG_SET_BITS(&hw->gpio_32_39_num[index-GPIO_32], GPIO_F_IO_MODE_SET(io_mode), GPIO_F_IO_MODE_EN_M) ;
}
...
static inline void gpio_ll_set_pull_mode(gpio_hw_t *hw, uint32 index, const gpio_config_t *config)
{
	...
	if (index < GPIO_32)
		REG_SET_BITS(&hw->gpio_0_31_num[index], GPIO_F_PULL_SET(pull_mode), GPIO_F_PULL_EN_M) ;
	else
		REG_SET_BITS(&hw->gpio_32_39_num[index-GPIO_32], GPIO_F_PULL_SET(pull_mode), GPIO_F_PULL_EN_M) ;
}
...
static inline void gpio_ll_set_func_mode(gpio_hw_t *hw, uint32 index, const gpio_config_t *config)
{
	...
	case GPIO_SECOND_FUNC_DISABLE:
		if (index < GPIO_32) {
			REG_CLR_BIT(&hw->gpio_0_31_num[index], BIT(6));
		} else {
			REG_CLR_BIT(&hw->gpio_32_39_num[index - 32], BIT(6));
		}
		break;
	case GPIO_SECOND_FUNC_ENABLE:
		if (index < GPIO_32) {
			REG_SET_BIT(&hw->gpio_0_31_num[index], BIT(6));
		} else {
			REG_SET_BIT(&hw->gpio_32_39_num[index - 32], BIT(6));
		}
		break;
	default:
		break;
...
static inline void gpio_ll_output_enable(gpio_hw_t *hw, uint32 index, uint32_t enable)
{
	...
	if (index < GPIO_32)
		hw->gpio_0_31_num[index].cfg.gpio_output_en = enable;
	else
		hw->gpio_32_39_num[index-GPIO_32].cfg.gpio_output_en = enable;
}

static inline void gpio_ll_input_enable(gpio_hw_t *hw, uint32 index, uint32_t enable)
{
	if (index < GPIO_32)
		hw->gpio_0_31_num[index].cfg.gpio_input_en = enable;
	else
		hw->gpio_32_39_num[index-GPIO_32].cfg.gpio_input_en = enable;
}

static inline void gpio_ll_pull_up_enable(gpio_hw_t *hw, uint32 index, uint32_t enable)
{
	if (index < GPIO_32)
		hw->gpio_0_31_num[index].cfg.gpio_pull_mode = enable;
	else
		hw->gpio_32_39_num[index-GPIO_32].cfg.gpio_pull_mode = enable;
}

static inline void gpio_ll_pull_enable(gpio_hw_t *hw, uint32 index, uint32_t enable)
{
	if (index < GPIO_32)
		hw->gpio_0_31_num[index].cfg.gpio_pull_mode_en = enable;
	else
		hw->gpio_32_39_num[index-GPIO_32].cfg.gpio_pull_mode_en = enable;
}

static inline void gpio_ll_second_function_enable(gpio_hw_t *hw, uint32 index, uint32_t enable)
{
	if (index < GPIO_32)
		hw->gpio_0_31_num[index].cfg.gpio_2_func_en = enable;
	else
		hw->gpio_32_39_num[index-GPIO_32].cfg.gpio_2_func_en = enable;
}

static inline void gpio_ll_monitor_input_value_enable(gpio_hw_t *hw, uint32 index, uint32_t enable)
{
	if (index < GPIO_32)
		hw->gpio_0_31_num[index].cfg.gpio_input_monitor = enable;
	else
		hw->gpio_32_39_num[index-GPIO_32].cfg.gpio_input_monitor = enable;
}

static inline void gpio_ll_set_capacity(gpio_hw_t *hw, uint32 index, uint32_t capacity)
{
	if (index < GPIO_32)
		hw->gpio_0_31_num[index].cfg.gpio_capacity = capacity;
	else
		hw->gpio_32_39_num[index-GPIO_32].cfg.gpio_capacity = capacity;
}

static inline void gpio_ll_set_perial_mode(gpio_hw_t *hw, uint32 index, uint32_t mode)
{
	if (index < GPIO_16)
		REG_MCHAN_SET_FIELD(index, &hw->gpio_0_15_func_mode, GPIO_F_PERIAL_MODE, mode);
	else if (index < GPIO_32)
		REG_MCHAN_SET_FIELD(index-GPIO_16, &hw->gpio_16_31_func_mode, GPIO_F_PERIAL_MODE, mode);
	else
		REG_MCHAN_SET_FIELD(index-GPIO_32, &hw->gpio_32_39_func_mode, GPIO_F_PERIAL_MODE, mode);
}

static inline uint32 gpio_ll_get_perial_mode(gpio_hw_t *hw, uint32 index)
{
	if (index < GPIO_16)
		return ((REG_MCHAN_GET_FIELD(index, &hw->gpio_0_15_func_mode, GPIO_F_PERIAL_MODE)));
	else if (index < GPIO_32)
		return ((REG_MCHAN_GET_FIELD(index-GPIO_16, &hw->gpio_16_31_func_mode, GPIO_F_PERIAL_MODE)));
	else
		return (REG_MCHAN_GET_FIELD(index-GPIO_32, &hw->gpio_32_39_func_mode, GPIO_F_PERIAL_MODE));
}

static inline uint32 gpio_ll_check_func_mode_enable(gpio_hw_t *hw, uint32 index)
{
	if (index < GPIO_32) {
		return (hw->gpio_0_31_num[index].cfg.gpio_2_func_en == 1);
	} else{
		return (hw->gpio_32_39_num[index-GPIO_32].cfg.gpio_2_func_en == 1);
	}
}
...
{
	if (index < GPIO_32) {
		return (hw->gpio_0_31_num[index].cfg.gpio_output_en == 0);
	} else{
		return (hw->gpio_32_39_num[index-GPIO_32].cfg.gpio_output_en == 0);
	}
}
...
{
	if (index < GPIO_32) {
		return (hw->gpio_0_31_num[index].cfg.gpio_input_en == 1);
	} else{
		return (hw->gpio_32_39_num[index-GPIO_32].cfg.gpio_input_en == 1);
	}
}

static inline void gpio_ll_spi_set_mode(gpio_hw_t *hw, uint32 mode)
{
	REG_SET_FIELD(&hw->spi_sdio_mode_cfg, GPIO_F_GPIO_SPI_SEL_MDOE, mode);
}

static inline void gpio_ll_sdio_set_mode(gpio_hw_t *hw, uint32 mode)
{
	REG_SET_FIELD(&hw->spi_sdio_mode_cfg, GPIO_F_GPIO_SDIO_SEL_MDOE, mode);
}

static inline void gpio_ll_set_output_value(gpio_hw_t *hw, uint32 index, bool value)
{
	if (index < GPIO_32)
		hw->gpio_0_31_num[index].cfg.gpio_output = value;
	else
		hw->gpio_32_39_num[index-GPIO_32].cfg.gpio_output= value;
}

static inline bool gpio_ll_get_input_value(gpio_hw_t *hw, uint32 index)
{
	if (index < GPIO_32)
		return hw->gpio_0_31_num[index].cfg.gpio_input ;
	else
		return hw->gpio_32_39_num[index - GPIO_32].cfg.gpio_input;
}
...
{
	if (index < GPIO_32)
		REG_SET_BIT(&hw->gpio_int_enable, 1 << index);
	else
...
{
	if (index < GPIO_32)
		REG_MCHAN_SET_FIELD(index, &hw->gpio_int_enable, GPIO_F_INT_EN, 0);
	else
