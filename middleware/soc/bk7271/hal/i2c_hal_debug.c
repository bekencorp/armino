...
static void i2c0_struct_dump(void)
{
	...
	SOC_LOGI("\n");
	SOC_LOGI("  fm_stereo=0x%x value=0x%x\n", &hw->fm_stereo, hw->fm_stereo.v);
	SOC_LOGI("    status:   %x\n", hw->fm_stereo.status);
	SOC_LOGI("    reserved: %x\n", hw->fm_stereo.reserved);
}

static void i2c1_struct_dump(i2c_id_t id)
{
	i2c1_hw_t *hw = (i2c1_hw_t *)I2C_LL_REG_BASE(id);
	SOC_LOGI("base=%x\r\n", (uint32_t)hw);
