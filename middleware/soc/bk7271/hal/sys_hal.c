...
int32 sys_hal_func_clk_pwr_down(uint32 param)
{
	...
	reg = REG_READ(ICU_R_FUNC_CLK_PWD);
	reg |= (param);
	REG_WRITE(ICU_R_FUNC_CLK_PWD, reg);
...
void sys_hal_clk_pwr_ctrl(dev_clk_pwr_id_t dev, dev_clk_pwr_ctrl_t power_up)
{
	...
	if(dev >= CLK_PWR_ID_GROUP_2_START)
	{
		v = icu_ll_get_peri_clk_pwr_status_group2(s_icu_hal.hw);
		bit = dev - CLK_PWR_ID_GROUP_2_START;
	}
	else
		v = icu_ll_get_peri_clk_pwr_status(s_icu_hal.hw);
	...
	if(dev >= CLK_PWR_ID_GROUP_2_START)
		icu_ll_set_peri_clk_pwr_group2(s_icu_hal.hw, v);
	else
