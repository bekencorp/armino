...
bk_err_t h264_hal_set_img_width(h264_hal_t *hal,uint32_t h264_resolution_type)
{
	int ret = 0;
	switch (h264_resolution_type){
		case QCIF:
			h264_ll_set_img_width(hal->hw,QCIF_WIDTH);
			h264_ll_set_img_height(hal->hw,QCIF_HEIGHT);
			h264_ll_set_mb_lines(hal->hw,QCIF_LINES);
			break;
		case CIF:
			h264_ll_set_img_width(hal->hw,CIF_WIDTH);
			h264_ll_set_img_height(hal->hw,CIF_HEIGHT);
			h264_ll_set_mb_lines(hal->hw,CIF_LINES);
			break;
		case VGA:
			h264_ll_set_img_width(hal->hw,VGA_WIDTH);
			h264_ll_set_img_height(hal->hw,VGA_HEIGHT);
			h264_ll_set_mb_lines(hal->hw,VGA_LINES);
			break;
		case FCIF:
			h264_ll_set_img_width(hal->hw,FCIF_WIDTH);
			h264_ll_set_img_height(hal->hw,FCIF_HEIGHT);
			h264_ll_set_mb_lines(hal->hw,FCIF_LINES);
			break;
		case HDTV_720P:
			h264_ll_set_img_width(hal->hw,HDTV_720P_WIDTH);
			h264_ll_set_img_height(hal->hw,HDTV_720P_HEIGHT);
			h264_ll_set_mb_lines(hal->hw,HDTV_720P_LINES);
			break;
		case HDTV_1080P:
			h264_ll_set_img_width(hal->hw,HDTV_1080P_WIDTH);
			h264_ll_set_img_height(hal->hw,HDTV_1080P_HEIGHT);
			h264_ll_set_mb_lines(hal->hw,HDTV_1080P_LINES);
			break;
		default:
			HAL_LOGI("invalid input image resolution \r\n");
			ret = 1;
			break;
	}
	return ret;
