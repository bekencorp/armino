...
static uint32_t dm_ble_event_cb(ble_event_enum_t notice, void *param)
{
    ...
    case BK_DM_BLE_EVENT_DISCONNECT:
    {
        ...
        os_printf("%s disconnect :conn_idx:%d,reason:%d\n", __func__, d_ind->conn_handle, d_ind->event_result);
        s_conn_ind = ~0;

        retval = bk_ble_set_advertising_enable(1, dm_ble_cmd_cb);

        if (retval != BK_ERR_BLE_SUCCESS)
        {
            os_printf("%s: bk_ble_set_advertising_enable failed. ret: 0x%04X\n", __func__, retval);
        }
    ...
    {
        os_printf("%s BK_DM_BLE_EVENT_CREATE_DB OK\n", __func__);
    ...
    case BK_DM_BLE_EVENT_CONN_UPDATA:
    {
        ...
        os_printf("%s BK_DM_BLE_EVENT_CONN_UPDATA:conn_interval:0x%04x, con_latency:0x%04x, sup_to:0x%04x\n", __func__,
...
int dm_ble_demo_main(void)
{
    ...
    if (0 != retval)
    {
        os_printf("%s: bk_ble_gatt_db_add_service() failed. Result: 0x%04X\n", __func__, retval);
        goto error;
    }
    else
    {
    ...
    if (retval != BK_ERR_BLE_SUCCESS)
    {
        os_printf("%s: bk_ble_set_advertising_params() failed. ret: 0x%04X\n", __func__, retval);
        goto error;
    }
    ...
    if (retval != BK_ERR_BLE_SUCCESS)
    {
        os_printf("%s: bk_ble_set_random_addr failed. ret: 0x%04X\n", __func__, retval);
        goto error;
    }
    ...
    if (ble_boarding_sema != NULL)
    {
        ...
        if (retval != kNoErr)
        {
            os_printf("%s: wait bk_ble_set_random_addr failed. ret: 0x%04X\n", __func__, retval);
            goto error;
        }
    }
    ...
    if (retval != BK_ERR_BLE_SUCCESS)
    {
        os_printf("%s: bk_ble_set_advertising_data failed. ret: 0x%04X\n", __func__, retval);
        goto error;
    }
    ...
    if (ble_boarding_sema != NULL)
    {
        ...
        if (retval != kNoErr)
        {
            os_printf("%s: wait bk_ble_set_advertising_data failed. ret: 0x%04X\n", __func__, retval);
            goto error;
        }
    }
    ...
    if (ble_boarding_sema != NULL)
    {
        ...
        if (retval != kNoErr)
        {
            os_printf("%s: wait bk_ble_set_advertising_enable failed. ret: 0x%04X\n", __func__, retval);
            goto error;
        }
    }
    ...
    if (handle->service_id == boarding_env.pri_service_handle)
    {
        ...
        case GATT_DB_CHAR_PEER_WRITE_REQ:
        {
            if (handle->char_id == boarding_env.chara_pass_handle)
            {
                ...
                if (strlen((char *)s_boarding_ssid) && strlen((char *)s_boarding_password))
                {
