...
#define BK_LOGE( tag, format, ... ) printf(format, ##__VA_ARGS__)
#define BK_LOGW( tag, format, ... ) printf(format, ##__VA_ARGS__)
#define BK_LOGI( tag, format, ... ) printf(format, ##__VA_ARGS__)
#define BK_LOGD( tag, format, ... ) printf(format, ##__VA_ARGS__)
#define BK_LOGV( tag, format, ... ) printf(format, ##__VA_ARGS__)
...
#define rtos_get_time()   0
#define BK_LOG_FORMAT(letter, format)  LOG_COLOR_ ## letter #letter " (%d) %s: " format LOG_RESET_COLOR

#if (LOG_LEVEL >= BK_LOG_ERROR)
#define BK_LOGE( tag, format, ... ) printf(BK_LOG_FORMAT(E, format), rtos_get_time(), tag, ##__VA_ARGS__)
#else
#define BK_LOGE( tag, format, ... )
#endif

#if (LOG_LEVEL >= BK_LOG_WARN)
#define BK_LOGW( tag, format, ... ) printf(BK_LOG_FORMAT(W, format), rtos_get_time(), tag, ##__VA_ARGS__)
#else
#define BK_LOGW( tag, format, ... )
#endif

#if (LOG_LEVEL >= BK_LOG_INFO)
#define BK_LOGI( tag, format, ... ) printf(BK_LOG_FORMAT(I, format), rtos_get_time(), tag, ##__VA_ARGS__)
#else
#define BK_LOGI( tag, format, ... )
#endif

#if (LOG_LEVEL >= BK_LOG_DEBUG)
#define BK_LOGD( tag, format, ... ) printf(BK_LOG_FORMAT(D, format), rtos_get_time(), tag, ##__VA_ARGS__)
#else
#define BK_LOGD( tag, format, ... )
#endif

#if (LOG_LEVEL >= BK_LOG_VERBOSE)
#define BK_LOGV( tag, format, ... ) printf(BK_LOG_FORMAT(V, format), rtos_get_time(), tag, ##__VA_ARGS__)
#else
...
void bk_mem_dump(const char* titile, int start, int len);
