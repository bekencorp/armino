...
static bk_err_t aud_asr_send_msg(audio_asr_op_t op)
{
	bk_err_t ret;
	...
	if (aud_asr_msg_que) {
		ret = rtos_push_to_queue(&aud_asr_msg_que, &msg, BEKEN_NO_WAIT);
		if (kNoErr != ret) {
...
static int aud_asr_handle(uint8_t *data, unsigned int len)
{
	...
	if (ring_buffer_get_fill_size(&asr_rb) >= 960) {
		...
		aud_asr_send_msg(AUDIO_ASR_PROCESS);
	}
...
static void aud_asr_main(void)
{
	...
	audio_asr_msg_t msg;
	while(1) {
		ret = rtos_pop_from_queue(&aud_asr_msg_que, &msg, BEKEN_WAIT_FOREVER);
		if (kNoErr == ret) {
			if (msg.op == AUDIO_ASR_PROCESS) {
			...
			} else if (msg.op == AUDIO_ASR_EXIT) {
				break;
			} else {
				LOGE("not support op: %d \n", msg.op);
			}
		}
	}

	/* delete msg queue */
	ret = rtos_deinit_queue(&aud_asr_msg_que);
	if (ret != kNoErr) {
...
bk_err_t wanson_asr_open(aud_intf_mic_type_t mic_type)
{
	...
	aud_intf_drv_setup.aud_intf_rx_spk_data = read_voice_data_from_sd;
...
bk_err_t wanson_asr_close(void)
{
	...
	aud_asr_send_msg(AUDIO_ASR_EXIT);
