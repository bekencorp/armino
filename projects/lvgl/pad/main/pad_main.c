...
static const lcd_open_t lcd_open =
{
    .device_ppi = PPI_480X480,
    .device_name = "st7701s",
};

static void cli_86box_cmd(char *pcWriteBuffer, int xWriteBufferLen, int argc, char **argv)
{
    os_printf("%s %d\r\n", __func__, __LINE__);
}
...
static int cli_86box_init(void)
{
...
#define PSRAM_FRAME_BUFFER ((0x60000000UL) + 5 * 1024 * 1024)
    ...
    {
        os_printf("%s draw_buf_2_1 alloc fail %d\n", __func__, lv_vnd_config.draw_pixel_size * sizeof(lv_color_t));
        return;
    }
