...
int doorbell_camera_turn_on(camera_parameters_t *parameters)
{
	...
	camera_config.width = parameters->width;
	camera_config.height = parameters->height;
	...
	{
		ret = media_app_transfer_open(db_device_info->camera_transfer_cb);
	}
	else
	{
...
void doorbell_audio_data_callback(uint8_t *data, uint32_t length)
{
	...
	if (ret != BK_OK)
	{
		LOGE("write speaker data fail %d\n", length, ret);
	}
}
