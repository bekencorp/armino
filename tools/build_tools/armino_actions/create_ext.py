...
def create_project(target_path, name):
    copy_tree(os.path.join(os.environ['ARMINO_PATH'], 'examples', 'templates', 'app-template'), target_path)
    main_folder = os.path.join(target_path, 'main')
    os.rename(os.path.join(main_folder, 'app_template_main.c'), os.path.join(main_folder, '.'.join((name + '_main', 'c'))))
    replace_in_file(os.path.join(main_folder, 'CMakeLists.txt'), 'app_template', name)
    replace_in_file(os.path.join(target_path, 'CMakeLists.txt'), 'app_template', name)

def create_component(target_path, name):
    copy_tree(os.path.join(os.environ['ARMINO_PATH'], 'examples', 'templates', 'component-template'), target_path)
    os.rename(os.path.join(target_path, 'component_src.c'), os.path.join(target_path, '.'.join((name, 'c'))))
    os.rename(os.path.join(target_path, 'include', 'component_src.h'),
              os.path.join(target_path, 'include', '.'.join((name, 'h'))))

    replace_in_file(os.path.join(target_path, '.'.join((name, 'c'))), 'component_src', name)
    replace_in_file(os.path.join(target_path, 'CMakeLists.txt'), 'component_src', name)
...
def action_extensions(base_actions, project_path):
    def create_new(action, ctx, global_args, **action_args):
        ...
        func_action_map = {'create-app': create_project, 'create-component': create_component}
        func_action_map[action](target_path, action_args['name'])

        print('The', get_type(action), 'was created in', os.path.abspath(target_path))

        # after the command execution, no other commands are accepted and armino.py terminates
        sys.exit(0)
        ...
        'actions': {
            'create-app': {
                ...
                'help': ('Create a new project with the name NAME specified as argument. '
                         'For example: '
                         '`armino.py create-application new_proj` '
                         ...
                         'After the execution armino.py terminates '
            ...
            'create-component': {
                ...
                'help': ('Create a new component with the name NAME specified as argument. '
                         'For example: '
                         '`armino.py create-component new_comp` '
